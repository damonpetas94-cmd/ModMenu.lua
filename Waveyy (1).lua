-- Waveyy.lua
-- loadstring(game:HttpGet("https://raw.githubusercontent.com/damonpetas94-cmd/ModMenu.lua/main/Waveyy.lua"))()
...
introGui.Name="._WaveyyyIntro" introGui.ResetOnSpawn=false
...
backdrop.BorderSizePixel=0 backdrop.Parent=introGui
...
local NUM_LINES=28 local lines={}
for i=1,NUM_LINES do
    local line=Instance.new("Frame")
    local xPos=(i-1)/NUM_LINES
    line.Size=UDim2.new(0,3,1.4,0)
    line.Position=UDim2.new(xPos,0,-0.2,0)
    line.BackgroundColor3=Color3.fromRGB(math.random(5,18),math.random(5,18),math.random(20,50))
    line.BorderSizePixel=0 line.Rotation=math.random(-8,8) line.Parent=waveContainer
    lines[i]={frame=line,baseX=xPos,baseRot=line.Rotation,offset=math.random(0,62)/10}
end

local titleLabel=Instance.new("TextLabel")
titleLabel.Size=UDim2.new(1,0,0,80) titleLabel.Position=UDim2.new(0,0,0.5,-40)
titleLabel.BackgroundTransparency=1 titleLabel.Text="Waveyy"
titleLabel.Font=Enum.Font.GothamBlack titleLabel.TextSize=52
titleLabel.TextColor3=Color3.fromRGB(30,60,220)
titleLabel.TextStrokeColor3=Color3.fromRGB(0,10,80) titleLabel.TextStrokeTransparency=0.3
titleLabel.TextTransparency=1 titleLabel.Parent=backdrop

local subLabel=Instance.new("TextLabel")
subLabel.Size=UDim2.new(1,0,0,30) subLabel.Position=UDim2.new(0,0,0.5,44)
subLabel.BackgroundTransparency=1 subLabel.Text="loading..."
subLabel.Font=Enum.Font.Gotham subLabel.TextSize=14
subLabel.TextColor3=Color3.fromRGB(60,100,200) subLabel.TextTransparency=1 subLabel.Parent=backdrop

local t=0 local waveConn
...
TweenService:Create(subLabel,TweenInfo.new(0.7),{TextTransparency=1}):Play()
...
--  STATE
-- ════════════════════════════════════════════════════════════
local state           = {fly=false,noclip=false,esp=false,aimbot=false,triggerbot=false}
local FLY_SPEED       = 80
local AIMBOT_SMOOTH   = 0.15
local TRIGGER_RANGE   = 5   -- studs — how close before triggerbot fires
local excludedPlayers = {}
local invisActive     = false

-- Keybinds stored as strings for easy display/update
local keybinds = {
    aimbot    = Enum.KeyCode.RightAlt,
    toggleGui = Enum.KeyCode.RightShift,
}
local remapping = nil  -- which keybind we're waiting on: "aimbot" or "toggleGui"
...
-- ════════════════════════════════════════════════════════════
--  TRIGGERBOT
--  Auto-clicks when your crosshair is over a player
-- ════════════════════════════════════════════════════════════
        ...
        if p ~= lp and not excludedPlayers[p.Name] and p.Character == hitChar then
            ...
            pcall(function()
                ...
                if tool then
                    ...
                    if remoteFire then remoteFire:FireServer() end
                    -- Also try ClickDetector method
...
local function teleportToPlayer(targetName)
    ...
    local targetRoot=target.Character:FindFirstChild("HumanoidRootPart") if not targetRoot then return false end
...
-- ════════════════════════════════════════════════════════════
--  UNLOCK ALL  (safe version - no crash)
-- ════════════════════════════════════════════════════════════
local function unlockAll()
    task.spawn(function()
        ...
        -- Max out player values safely (most common unlock method in Roblox games)
        pcall(function()
        ...
        -- Fire remotes one at a time with small delays to avoid crash
        pcall(function()
            local unlockWords={"unlock","give","grant","skin","charm","finisher","emote","cosmetic","item","award","equip"}
            local fired = 0
            for _,v in ipairs(rs:GetDescendants()) do
                if fired > 30 then break end  -- cap at 30 remotes max
                local n=v.Name:lower()
                if v:IsA("RemoteEvent") then
                    for _,w in ipairs(unlockWords) do
                        if n:find(w) then
                            pcall(function() v:FireServer() end)
                            task.wait(0.05)  -- small delay between each
                            fired = fired + 1
                            break
                        end
                    end
...
if not keyType or keyType=="expired" then
    ...
    kbrand.TextStrokeColor3=Color3.fromRGB(0,10,80) kbrand.TextStrokeTransparency=0.4 kbrand.Parent=card
    ...
    kbtn.MouseButton1Click:Connect(function()
        ...
        elseif result=="expired" then
            kstatus.Text="Key expired! Get a new one." kentry.Text=""
        else
...
MainTab:CreateToggle({Name="Fly",CurrentValue=false,Flag="FlyToggle",
    Callback=function(v) state.fly=v if v then enableFly() else disableFly() end end})

MainTab:CreateSlider({Name="Fly Speed  [500 = invisible]",Range={1,500},Increment=1,CurrentValue=80,Flag="FlySpeed",
    Callback=function(v) FLY_SPEED=v end})

MainTab:CreateDivider()

MainTab:CreateToggle({Name="NoClip",CurrentValue=false,Flag="NoclipToggle",
    Callback=function(v) state.noclip=v end})

MainTab:CreateDivider()

MainTab:CreateToggle({Name="ESP",CurrentValue=false,Flag="ESPToggle",
    Callback=function(v) state.esp=v if v then enableESP() else disableESP() end end})

MainTab:CreateDivider()

MainTab:CreateToggle({Name="Aimbot (auto lock-on)",CurrentValue=false,Flag="AimbotToggle",
    Callback=function(v) state.aimbot=v end})

MainTab:CreateSlider({Name="Aimbot Smoothness  [1=slow 20=snap]",Range={1,20},Increment=1,CurrentValue=3,Flag="AimbotSmooth",
    Callback=function(v) AIMBOT_SMOOTH=v/20 end})

MainTab:CreateDivider()

MainTab:CreateToggle({Name="Triggerbot (auto shoot on target)",CurrentValue=false,Flag="TriggerToggle",
    Callback=function(v) state.triggerbot=v end})

MainTab:CreateSlider({Name="Triggerbot Range (studs)",Range={1,50},Increment=1,CurrentValue=5,Flag="TriggerRange",
    Callback=function(v) TRIGGER_RANGE=v end})

-- ── Unlock Tab ────────────────────────────
local UnlockTab=Window:CreateTab("🔓  Unlock",4483362458)

UnlockTab:CreateParagraph({Title="How it works",Content="Tries to unlock cosmetics by modifying local values and firing unlock remotes in your game. Results depend on how the game is built."})

UnlockTab:CreateButton({Name="🎨  Unlock All Skins",
    Callback=function() unlockAll() Rayfield:Notify({Title="Skins",Content="Attempting to unlock all skins...",Duration=4}) end})
UnlockTab:CreateButton({Name="✨  Unlock All Charms",
    Callback=function() unlockAll() Rayfield:Notify({Title="Charms",Content="Attempting to unlock all charms...",Duration=4}) end})
UnlockTab:CreateButton({Name="💀  Unlock All Finishers",
    Callback=function() unlockAll() Rayfield:Notify({Title="Finishers",Content="Attempting to unlock all finishers...",Duration=4}) end})
UnlockTab:CreateButton({Name="💃  Unlock All Emotes",
    Callback=function() unlockAll() Rayfield:Notify({Title="Emotes",Content="Attempting to unlock all emotes...",Duration=4}) end})
UnlockTab:CreateDivider()
UnlockTab:CreateButton({Name="🔓  Unlock EVERYTHING",
    Callback=function() unlockAll() Rayfield:Notify({Title="Unlock All",Content="Attempting to unlock everything...",Duration=5}) end})

-- ── Teleport Tab ──────────────────────────
local TpTab=Window:CreateTab("🌀  Teleport",4483362458)
TpTab:CreateParagraph({Title="Teleport to Player",Content="Click a player button to instantly teleport to them."})
local tpToggles={}
local function refreshTpList()
    for _,p in ipairs(Players:GetPlayers()) do
        if p~=lp and not tpToggles[p.Name] then
            tpToggles[p.Name]=true
            TpTab:CreateButton({Name="🌀  "..p.Name,Callback=function()
                local ok=teleportToPlayer(p.Name)
                if ok then Rayfield:Notify({Title="Teleported!",Content="Warped to "..p.Name,Duration=3})
                else Rayfield:Notify({Title="Failed",Content=p.Name.." not found!",Duration=3}) end
            end})
        end
    end
end
...
ExclTab:CreateParagraph({Title="Aimbot + Triggerbot Exclusions",Content="Toggle a player OFF to exclude them from aimbot and triggerbot."})
local exclToggles={}
...
ExclTab:CreateButton({Name="🔄  Refresh List",Callback=function()
    refreshExclusions()
    Rayfield:Notify({Title="Refreshed",Content="Player list updated!",Duration=3})
end})
...
KbTab:CreateParagraph({
    Title="How to remap",
    Content="1. Click a Remap button below\n2. Press ESC to close Rayfield first if needed\n3. Then press the key you want"
})

-- We show current key in a label that updates
local aimbotKeyLabel = KbTab:CreateLabel("Aimbot key: [ "..keybinds.aimbot.Name.." ]")
KbTab:CreateButton({Name="🎯  Remap Aimbot Key",Callback=function()
    remapping="aimbot"
    Rayfield:Notify({Title="Remap",Content="Press any key to set aimbot toggle...",Duration=5})
end})

KbTab:CreateDivider()

local guiKeyLabel = KbTab:CreateLabel("Show/Hide GUI key: [ "..keybinds.toggleGui.Name.." ]")
KbTab:CreateButton({Name="👁  Remap Show/Hide GUI Key",Callback=function()
    remapping="toggleGui"
    Rayfield:Notify({Title="Remap",Content="Press any key to set GUI toggle...",Duration=5})
end})

-- ── Info Tab ──────────────────────────────
local InfoTab=Window:CreateTab("ℹ  Info",4483362458)
InfoTab:CreateParagraph({Title="Waveyy",Content="Made by Damon\nPrivate use only"})
InfoTab:CreateParagraph({Title="Key Status",Content=isPremium and "Lifetime (Premium) access!" or "Key expires 24h after first use."})
InfoTab:CreateParagraph({Title="Controls",Content="Fly: WASD | Space=up | Ctrl=down\nSpeed 500 = invisible troll mode\nTriggerbot: auto shoots when crosshair is on a player"})
InfoTab:CreateParagraph({Title="Compatibility",Content="Xeno, Delta, Synapse X, KRNL, Fluxus, Evon, Codex, Arceus X"})

-- ════════════════════════════════════════════════════════════
--  INPUT HANDLER  (runs at top level, always catches input)
-- ════════════════════════════════════════════════════════════
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    -- Don't block if typing in a text box
    if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
    if gameProcessed then
        -- Still allow remapping even when GUI is focused
        if remapping then
            local key = input.KeyCode
            if key == Enum.KeyCode.Unknown then return end
            keybinds[remapping] = key
            Rayfield:Notify({Title="Key Set!", Content=remapping.." key: "..key.Name, Duration=3})
            -- Update labels
            pcall(function()
                if remapping == "aimbot" then
                    aimbotKeyLabel:Set("Aimbot key: [ "..key.Name.." ]")
                else
                    guiKeyLabel:Set("Show/Hide GUI key: [ "..key.Name.." ]")
                end
            end)
            remapping = nil
        end
    ...
    -- Toggle GUI
    if key == keybinds.toggleGui then
        for _,v in ipairs(lp.PlayerGui:GetChildren()) do
            if v.Name:find("Rayfield") or v.Name:find("Waveyy") then
